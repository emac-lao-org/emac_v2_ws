  ...
  <div>
    ...
    <div v-else id="body">
      <div id="bg" class="mx-10">
        <div>
              ...
              <v-row class="d-flex justify-end">
                <table style="width: 600px">
                  <tbody>
                    ...
                    <tr>
                      <td class="text-uppercase" id="detail_right">
                        ວັນທີເຮັດການຂົນສົ່ງ/Manifest date:
                      </td>
                      <td id="detail_right_result">
                        {{ manifestDate }}
                      </td>
          ...
          <div>
            ...
            <v-divider class="mb-6" style="border: 1px solid blue"> </v-divider>
          </div>
          ...
          <page-disposal
            ...
            :waste_operations="waste_operations"
          />
          <v-divider class="my-2" style="border: 1px solid blue"> </v-divider>
          <v-row >
            <v-col>
              <h2 id="docHeader" class="text-uppercase">
                  ...
                  <tr>
                    <td style="width: 65%" id="remove_border">
                      <table style="width: 100%">
                        <tr>
                          <td id="remove_border">
                            <b id="card_stt_details">ສະຖານະ / STATUS</b>
                          </td>
                      ...
                      <table style="width: 100%">
                        <tr>
                          ...
                          <td class="status_detail">
                            ...
                            <b id="detail_result_in" v-else> </b>
                          </td>
                        </tr>
                        <tr>
      ...
      <v-card-actions class="mx-4 my-6">
        ...
        <v-btn id="print" color="primary" class="mx-4 my-4" @click="print">
          Print
        </v-btn>
        <v-btn
          id="print"
    ...
    <center>
      <p class="primary--text">
        Print by WS: {{ this.ws_name }} {{ this.currentDate }}
      </p>
    </center>
  </div>
</template>
...
export default {
  ...
  methods: {
    async getToken() {
      ...
      this.ws_name = result?.claims?.name || "";
    ...
    getWasteOpt() {
      return this.$apollo
        .query({
          query: require("~/gql/queries/manifest/getWasteOpt").getWasteOpt,
          variables: {
            id: this.id ? this.id : 0,
          },
        }).then((data) => {
          this.waste_operations = data?.data?.emac_waste_operations || [];
          // console.log("🚀 ~ file: index.vue:485 ~ getWasteOpt ~  this.waste_operations:",  this.waste_operations)

          const checkR_D = this.waste_operations.map((item) => {
            return (item.r_d_code_operations || []).map((item) => {
          ...
          const checkR_D_null = checkR_D.map((item) => {
          ...
          if (checkR_D.length >= 1 && checkR_D_null.length >= 1) {
            for (let i = 0; i < checkR_D.length; i++) {
    ...
    getManifestById() {
      return this.$apollo
        ...
        .then((data) => {
          const manifest = data?.data?.manifest?.[0];
          if (!manifest) {
            this.data = [];
            this.data_wt = [];
            return;
          ...
          this.data = manifest;
          this.tos = manifest?.type_of_service?.name_short || "";
          this.wd_stt = manifest?.wd_stt || 0;
          this.wg_stt = manifest?.wg_stt || 0;
          this.data_wd = manifest?.disposal || [];
          this.data_vill_wd = this.data_wd?.custom_address || "";
          this.data_wg = data?.data?.gen?.[0] || [];
          this.note = manifest?.note || "";
          this.data_wt = manifest?.transporter_operations || [];
          ...
          this.vill_id = this.data_wg?.vill_id || 10101;
          this.vill_wd = manifest?.disposal?.vill_id || 10101;

          if (this.data_wt.length > 0) {
            this.vill_wt1 =
              manifest?.transporter_operations?.[0]?.transporter?.vill_id ||
              10101;

            // this.transporter_operations_1 = this.data_wt[0]
            // console.log('this.transporter_operations_1 :>> ', this.transporter_operations_1)
            if (manifest?.transporter_operations?.length == 2) {
              this.vill_wt2 =
                manifest?.transporter_operations?.[1]?.transporter?.vill_id ||
                10101;
              // this.transporter_operations_2 =
              //   this.data.transporter_operations[1].vehicle
            }
            if (manifest?.transporter_operations?.length == 3) {
              this.vill_wt3 =
                manifest?.transporter_operations?.[2]?.transporter?.vill_id ||
                10101;
    ...
    async getCustomAddress() {
      return this.$apollo
        ...
        .then(() => {
          ...
          // .then((data) => {
          //   // console.log('Data :>> ', data.data.emac_transporters)
          //   this.address_wd = data.data.emac_villages[0];
          //   // console.log('this.address_wd :>> ', this.address_wd)

          //   this.data_vill_wd =
          //     this.address_wd.vill_name_lo +
          //     ", " +
          //     this.address_wd.district.dr_name_lo +
          //     ", " +
          //     this.address_wd.district.province.pr_name_lo;

          //   // console.log("this.data_vill_wd :>> ", this.data_vill_wd);
          // });
        })
        .catch((err) => {
          // console.log("err :>> ", err);
    ...
    getInfo() {
      return this.$apollo
        ...
        .then((res) => {
          const manifest = res?.data?.manifest?.[0];
          if (!manifest) {
            return;
          }

          const wrDate = manifest?.qr_codes?.[0]?.wr_date;
          ...
          this.url_gen =
            ...
            "&module=WR";
          // console.log(
          //   "🚀 ~ file: pageGenerator.vue ~ line 360 ~ .then ~ this.url_gen",
          //   this.url_gen
          // );
          ...
          QRCode.toDataURL(this.id ? this.id : this.$route.query.id, {
  ...
  computed: {
    ...
    requestDate() {
      return moment(this.data.manifest_date).format("DD-MM-YYYY");
    },
    manifestDate() {
      return moment(this.data.relocate_date).format("DD-MM-YYYY");
    },
  },
  async created() {
    this.loading = true;
