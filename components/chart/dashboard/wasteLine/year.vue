  ...
  computed: {
     range: {
      ...
      set(value) {
        ...
        console.log("🚀 ~ file: custom.vue:179 ~ set ~ this.from", this.from);
        this.to = value.end;
        console.log("🚀 ~ file: custom.vue:181 ~ set ~ this.to", this.to);
        for (let x = 0; x < 2; x++) {
          this.getDataSetHz();
          this.getDataSetNHz();
        }
    ...
    from() {
      const dateTo = moment()
        ...
        .endOf("month")
        .format("YYYY-MM-DD");
      const dateSub = moment().subtract(12, "month").format("YYYY-MM-01");
      this.dateFrom[0] = {
        from: `${dateSub}`,
        to: `${dateTo}`,
