        ...
        <v-col align-self="center" cols="auto" class="d-flex">
          <v-tooltip top>
            <template v-slot:activator="{ on, attrs }">
              <v-btn icon @click="getWDUsers" color="primary" class="mr-2" v-bind="attrs" v-on="on">
  ...
  methods: {
    buildWhere() {
      ...
      if (this.search) {
        where._or = [
          ...
          { disposal: { emac_id: { _ilike: `%${this.search}%` } } }
        ];
      }
      return where;
    },
    async getWDUsers() {
      ...
      try {
        const countRes = await this.$apollo.query({
          query: require("~/gql/queries/users_management/getWDUsers").getWDUsersCount,
          variables: { where: whereCondition },
          fetchPolicy: "network-only",
        });
        this.totalCount = countRes.data.emac_wd_users_aggregate.aggregate.count;

        const { data } = await this.$apollo.query({
          query: require("~/gql/queries/users_management/getWDUsers").getWDUsers,
          variables: { limit, offset, where: whereCondition },
          fetchPolicy: "network-only",
        });

        this.dataUserWd = data.emac_wd_users;
      } catch (error) {
        console.error("Error fetching WD users: ", error);
      }
    },
    convertToCSV(arr) {
    ...
    async startExport(format) {
      ...
      try {
        while (offset < this.totalCount) {
          ...
          for (let item of chunk) {
            ...
            allMappedData.push({
              ...
              "Email": item.email || "-",
              "Telephone": item.tel || "-",
              "Enterprise Name": item.disposal?.dis_name || "-",
              "EMCD ID": item.disposal?.emac_id || "-",
              "Status": status_txt
            });
            this.exportProgress++;
          }
        ...
        const fileName = `WDUsers_${yyyy}-${mm}-${dd}`;

        if (this.exportFormat === 'excel') {
          const XLSX = await import('xlsx');
          const worksheet = XLSX.utils.json_to_sheet(allMappedData);
          const workbook = XLSX.utils.book_new();
          XLSX.utils.book_append_sheet(workbook, worksheet, "Waste Disposal Users");
