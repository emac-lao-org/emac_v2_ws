...
<script>
import getWGUsersPer from "~/mixins/getWGUsersPer";
import update_wg_user from "~/gql/mutations/users_management/updateWGUserPer";
import delete_wg_user from "~/gql/mutations/users_management/deleteWGUserPer";
import gql from "graphql-tag";
export default {
  mixins: [getWGUsersPer],
  data() {
    return {
      firstName: "",
      lastName: "",
  ...
  methods: {
    ...
    update() {
      this.$apollo
        .mutate({
          mutation: gql`
            ${update_wg_user.updateWGUserPer}
          `,
          variables: {
            id: this.edit.id,
            name: this.edit.name,
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.$apollo
            .mutate({
              mutation: gql`
                ${delete_wg_user.deleteWGUserPer}
              `,
              variables: {
                id: item.id,
              },
