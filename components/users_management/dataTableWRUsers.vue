        ...
        <v-col align-self="center" cols="auto" class="d-flex">
          <v-tooltip top>
            <template v-slot:activator="{ on, attrs }">
              <v-btn icon @click="getWRUsers" color="primary" class="mr-2" v-bind="attrs" v-on="on">
...
<script>
import update_wr_user from "~/gql/mutations/users_management/updateWRUser";
import delete_wr_user from "~/gql/mutations/users_management/deleteWRUser";
...
export default {
  data() {
    return {
      ...
      //     value: "regulator.regulator_name",
      //   },
      //   {
      //     text: "Status",
  ...
  methods: {
    buildWhere() {
      ...
      if (this.search) {
        where._or = [
          ...
          { regulator: { regulator_name: { _ilike: `%${this.search}%` } } },
          { regulator: { emac_id: { _ilike: `%${this.search}%` } } }
        ];
      }
      return where;
    },
    async getWRUsers() {
      ...
      try {
        const countRes = await this.$apollo.query({
          query: require("~/gql/queries/users_management/getWRUsers").getWRUsersCount,
          variables: { where: whereCondition },
          fetchPolicy: "network-only",
        });
        this.totalCount = countRes.data.emac_wr_users_aggregate.aggregate.count;

        const { data } = await this.$apollo.query({
          query: require("~/gql/queries/users_management/getWRUsers").getWRUsers,
          variables: { limit, offset, where: whereCondition },
          fetchPolicy: "network-only",
        });

        this.dataUserWr = data.emac_wr_users;
      } catch (error) {
        console.error("Error fetching WR users: ", error);
      }
    },
    convertToCSV(arr) {
    ...
    async startExport(format) {
      ...
      try {
        while (offset < this.totalCount) {
          ...
          for (let item of chunk) {
            ...
            allMappedData.push({
              ...
              "Email": item.email || "-",
              "Telephone": item.tel || "-",
              "Enterprise Name": item.regulator?.regulator_name || "-",
              "EMCD ID": item.regulator?.emac_id || "-",
              "Status": status_txt
            });
            this.exportProgress++;
          }
        ...
        const fileName = `WRUsers_${yyyy}-${mm}-${dd}`;

        if (this.exportFormat === 'excel') {
          const XLSX = await import('xlsx');
          const worksheet = XLSX.utils.json_to_sheet(allMappedData);
          const workbook = XLSX.utils.book_new();
          XLSX.utils.book_append_sheet(workbook, worksheet, "Waste Regulator Users");
